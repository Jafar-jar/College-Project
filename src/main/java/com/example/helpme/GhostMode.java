...
public class GhostMode extends ImageView implements Collision {
    private double x = (Math.random()*1080)+1;
    private double y = (Math.random()*720)+1;

    public GhostMode(){
        super();
        setImage(new Image(Objects.requireNonNull(getClass().getResource("ghostMode.png")).toExternalForm()));
        this.setFitWidth(40);
